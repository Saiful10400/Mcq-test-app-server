...
const updateLinkVisit = catchAsync(async (req: Request, res: Response) => {
  const data = await examService.updateLInkVisit(req.params.id);
  sendResponse(res, {
    data,
    success: true,
    statusCode: httpStatus.OK,
    message: "link visition status updated.",
  });
});

const examController = { createExam, findAExam, findAllExam, updateLinkVisit };
